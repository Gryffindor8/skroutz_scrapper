...
def ReadXml():
    ...
    for product in products:
        ...
        query = "INSERT INTO `products`(`name`, `category`, `brand`, `pid`, `mpn`, `price`, `updatedPrice`, `imgurl`) VALUES ('" + productname + "'," + str(
            categoryid) + "," + str(brandid) + ",'" + pid + "','" + mpn + "'," + str(price) + "," + str(
            price) + ", '" + imgurl + "')"
        cursor.execute(query)
        db.commit()
...
def Scrap():
    ...
    cursor.execute(
        "CREATE TABLE `competitors` ( `id` INT NOT NULL AUTO_INCREMENT, `product` INT NOT NULL, `storename` VARCHAR("
        "200) NOT NULL, `place` INT NOT NULL, `totalnum` INT NOT NULL, `price` VARCHAR(50) NOT NULL, `shipping` "
        "VARCHAR(50) NOT NULL, `delivery` VARCHAR(50) NOT NULL, `availability` VARCHAR(1000) NOT NULL, PRIMARY KEY ("
        ...
        try:
            ...
            for product_card in product_cards:
                ...
                try:
                    ...
                    continue
                query = "INSERT INTO `competitors`(`product`, `storename`, `place`, `totalnum`, `price`, `shipping`, " \
                        "`delivery`, `availability`)" \
                        "VALUES(" + str(product_id) + ", '" + store_name + "', " + str(i + 1) + ", " + str(
                    num_cards) + ", '" + price + "', '" + shipping + \
